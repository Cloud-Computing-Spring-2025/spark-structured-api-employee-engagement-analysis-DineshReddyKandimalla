...
def load_data(spark, file_path):
    ...
    df = spark.read.csv("/workspaces/spark-structured-api-employee-engagement-analysis-DineshReddyKandimalla/input/employee_data.csv", header=True, schema=schema)
    return df
...
def compare_engagement_levels(df):
    ...
    result_df = df.groupBy("JobTitle") \
        .agg(spark_round(avg("EngagementScore"), 2).alias("AverageEngagementScore"))
...
def main():
    ...
    # Define file paths
    input_file = "/workspaces/spark-structured-api-employee-engagement-analysis-DineshReddyKandimalla/input/employee_data.csv"
    output_file = "/workspaces/spark-structured-api-employee-engagement-analysis-DineshReddyKandimalla/Outputs/task3/engagement_levels_job_titles.csv"
